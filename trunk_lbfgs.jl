...
# import JSOSolvers: krylov_workspace
# import JSOSolvers: TrunkSolver as JSOTrunkSolver
...
# A::AbstractLinearOperator, b::AbstractVector{T}; itmax::Int=0
...
mutable struct LBFGSParams
    mem::Int
    scaling::Bool
end

# function krylov_workspace(::Val{:lbfgs}, n::Int, m::Int, V::Type{<:AbstractVector}; params::LBFGSParams=LBFGSParams(3, true))
#     # global LBFGS_MEMORY
#     # global LBFGS_SCALING

#     println("memory= ", params.mem)
#     println("scaling= ", params.scaling)

#     LBFGSWorkspace{eltype(V), eltype(V), V}(n, params.mem; scaling=params.scaling) # LBFGSWorkspace{eltype(V), eltype(V), V}(n, m)
# end
...
#     subsolver::Symbol=:cg, 
#     mem::Int=2,
#     scaling::Bool=true,) where {T, V<:AbstractVector{T}}
...
#         lbfgs_params = LBFGSParams(mem, scaling)
#         # function Krylov.krylov_workspace(::Val{:lbfgs}, n::Int, m::Int, V::Type{<:AbstractVector})
#         #     LBFGSWorkspace{eltype(V), eltype(V), V}(n, lbfgs_params.mem; lbfgs_params.scaling) # LBFGSWorkspace{eltype(V), eltype(V), V}(n, m)
#         # end

#         local krylov_wrapper(::Val{:lbfgs}, n::Int, m::Int, V::Type{<:AbstractVector}) = krylov_workspace(Val(:lbfgs), n, m, V; params=lbfgs_params)

#         return JSOTrunkSolver(nlp; subsolver=subsolver)
#     end
