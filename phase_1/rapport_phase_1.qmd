...
function lbfgs_trust_region(nlp; eps_a=1.0e-5, eps_r=1.0e-5, mem=5, max_iter_ratio=10)
    ...
    # Hk = I(dim)
    Hk = InverseLBFGSOperator(dim, mem = mem, scaling = true)  # par défaut, mem = 5
    ...
    for k in 1:max_iter
        ...
        @printf("|  k = %3d  |  grad_norm = %10.3e  |  fx = %10.3e  |  rho= %10.2f  |  deltak= %10.3e \n", k, grad_norm, fk, rho, deltak)

        # check convergence crieteria
        if grad_norm <= eps_a + eps_r*g0_norm
            println("Convergence reached! ||gk|| = ", grad_norm)
            break
        end
...
x_star = lbfgs_trust_region(models[1], mem=5)
