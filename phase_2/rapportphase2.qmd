...
using LinearAlgebra, ADNLPModels, NLPModels , Printf, Krylov, LinearOperators, SuiteSparseMatrixCollection, MatrixMarket 
using SolverCore
...
function cg_lbfgs(A, b, name, listmem, atol, rtol)
  ...
  f(x) = dot(0.5 .* x, A*x) - dot(b,x)
  model = ADNLPModel(f, zeros(n))

  #(xlbfgs,statslbfgs) = lbfgsbasic(A, b; atol=atol, rtol=rtol, mem = 1)
  (xlbfgs,statslbfgs) = lbfgsnew(model,A,b; mem = 1)
  gr()
  ...
  for mem in listmem
    (xlbfgs,statslbfgs) = lbfgsnew(model,A,b; mem = mem)  
...
function diom_lbfgs(A, b, name, listmem, atol, rtol)
  f(x) = dot(0.5 .* x, A*x) - dot(b,x)
  model = ADNLPModel(f, zeros(n))
  (xlbfgs,statslbfgs) = lbfgsnew(model,A,b; atol=atol, rtol=rtol, mem = 2)  
  ...
  for mem in listmem
    p = round(Int64, 100 * mem / n)
    if mem > 2 && p > 0
    (xlbfgs,statslbfgs) = lbfgsnew(model,A,b; atol=atol, rtol=rtol, mem = mem)  
...
function orthogonality_loss(A, b; maxiter=10, mem=1)
    ...
    model = ADNLPModel(f, zeros(n))
    for i in 2:maxiter
        (xlbfgs,statslbfgs) =  lbfgsnew(model,A,b; mem=mem, itmax = i)
        U = tril(statslbfgs.PAP, -1)
        if i == 5 && mem==1
        println(U)
        end                        # partie triangulaire inférieure stricte
        I_U = I + U         # U + I
        loss = norm(U,Inf)        # || (U + I)^(-1) * U ||_2
        push!(Loss, loss)              
    end        
    return Loss
end
